...
static void KS0108_setDB(uint8_t data) {
  uint8_t i;
  for (i = 0; i < 8; i++) {
    GPIO_setPinLevel(glcd_config->data_port, glcd_config->DB[i],
                     (GPIO_OUTPUT_t)GET_BIT(data, i));
  }
}

void KS0108_sendCommand(uint8_t cmd, uint8_t chip) {
  GPIO_setPinLevel(glcd_config->control_port, glcd_config->DI,
                   GPIO_LOW); // Command mode
  GPIO_setPinLevel(glcd_config->control_port, glcd_config->RW,
                   GPIO_LOW); // Write mode
  ...
  GPIO_setPinLevel(glcd_config->control_port, glcd_config->EN,
                   GPIO_HIGH); // Pulse enable
  vTaskDelay(1);
...
void KS0108_init(KS0108_config_t *config) {
  ...
  GPIO_setPinDirection(glcd_config->control_port, glcd_config->RST,
                       GPIO_OUTPUT);
  ...
  for (i = 0; i < 8; i++) {
    GPIO_setPinDirection(glcd_config->data_port, glcd_config->DB[i],
                         GPIO_OUTPUT);
  }

  // Reset the LCD
  // initalizing the screen
  GPIO_setPinLevel(glcd_config->control_port, glcd_config->RST,
                   GPIO_LOW); // Reset LCD
  vTaskDelay(pdMS_TO_TICKS(10));
  GPIO_setPinLevel(glcd_config->control_port, glcd_config->RST,
                   GPIO_HIGH); // Release reset
...
void KS0108_clearScreen(void) {
  ...
  for (page = 0; page < 8; page++) {
    KS0108_sendCommand((uint8_t)(0xB8 + page),
                       1); // Set page address (left half)
    KS0108_sendCommand((uint8_t)(0xB8 + page),
                       2);       // Set page address (right half)
